...
Since the monitor runs as SYSTEM, it can't just check `$env:USERPROFILE` for desktop mode — it needs to enumerate all profiles under `C:\Users\` to find every instance. The monitor checks all of them.

### What It Checks

For each status XML found, the monitor evaluates four things:
...
XML candidates to check: 4
Found XML: C:\ProgramData\Datto\Common\Status Report - Datto File Protection Server.xml
--- Evaluating: C:\ProgramData\Datto\Common\Status Report - ... ---
